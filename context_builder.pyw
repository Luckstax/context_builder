...
def load_model(lms: str, logger: Logger):
    model_key = cfg("lms_model_id")
    ctx_len   = cfg("lms_context_length")
...
def ensure_lmstudio(logger: Logger):
    ...
    if not is_lms_running():
        ...
        if model_short not in ls.stdout.lower():
            spec = cfg("lms_model_id")
            if cfg("lms_model_quant"):
                spec = f"{spec}@{cfg('lms_model_quant')}"
...
def _call_merge(client, series, items, count_desc, logger, label):
